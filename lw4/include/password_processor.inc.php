<?php
    ...
    function checkPasswordStrength($password) {
        ...
        for ($i = 0; $i < $arrayLength; ++$i) {
            if (in_array($verifiableArray[$i], $repetitionCheckingArray)) { // количество повторяющихся символов
                $repeatCounter++;
                if (!in_array($verifiableArray[$i], $repetitionArray)) { // символы которые повторяются
